...
public class ShooterSubsystem extends SubsystemBase {
  ...
  public ShooterSubsystem(SwerveSubsystem swerveSubsystem, NeoPixelSubsystem neopixelSubsystem) {
    ...
    this.neopixelSubsystem = neopixelSubsystem;
  ...
  public Command FeedCommand(boolean requireShooterAtTargetRPM) {
    return Commands.runEnd(
        () -> {
  ...
  public double getShooterDistanceToHub() {
    Pose2d robotPose = swerveSubsystem.getRobotPose();
    double dist = AutoHelper.GetShooterDistanceToHub(robotPose);
  ...
  private void toggleFeeder(boolean state) {
    feederState = state;
    neopixelSubsystem.setConditionState(NeoPixelCondition.SHOOTING, state);

    // feederMotor.getClosedLoopController().setSetpoint(state ?
    // Constants.ShooterConstants.Feeder_TargetVoltage : 0, ControlType.kVoltage);
  ...
  public void periodic() {
    ...
    if (feederMotor.getOutputCurrent() > 70) {
      if (onePeriodicCheck) {
        ...
        onePeriodicCheck = true;
    } else
      onePeriodicCheck = false;

    if (feederState && feederUnlockAfter < now)
      feederMotor.getClosedLoopController().setSetpoint(Constants.ShooterConstants.Feeder_TargetRPM,
  ...
  public void simulationPeriodic() {
    if (Math.abs(getUpperMotorRPM() - upperMotorTargetRPM) > Constants.ShooterConstants.RPM_Tolerance)
      upperMotorSimRPM += Math.signum(upperMotor.getClosedLoopController().getSetpoint() - upperMotorSimRPM) * 0.02
          * 4000;

    if (Math.abs(getLowerMotorRPM() - lowerMotorTargetRPM) > Constants.ShooterConstants.RPM_Tolerance)
      lowerMotorSimRPM += Math.signum(lowerMotor.getClosedLoopController().getSetpoint() - lowerMotorSimRPM) * 0.02
          * 4000;
 ...
 * };
 * 
 * private final SendableChooser<ShooterDistCalcMode> distCalcModeChooser = new
 * SendableChooser<ShooterDistCalcMode>();
 * private final DoubleEntry manualShooterDistanceEntry =
 * EntryUtils.createDoubleEntry("Shooter/ManualDistance", 2);
 * 
 * private SparkMax upperMotor;
 * private SparkMax lowerMotor;
 * private SparkMax feederMotor;
 ...
 * if(selectedSlot == -1)
 * {
 * Logging.stickyError("Shooter Calibration Error",
 * "An error occured while reading the selected shootercalib slot");
 * } else {
 * ShooterCalibration loadedCalibration =
 * ShooterCalibration.loadFromSlot(selectedSlot);
 * if(loadedCalibration == null && selectedSlot != 0)
 * {
 * Logging.stickyError("Shooter Calibration Error",
 * "An error occured while reading shootercalib slot " + selectedSlot);
 * }
 * else if(loadedCalibration != null) {
 * calibration = loadedCalibration;
 * Logging.infoMsg("Shooter Calibration Loaded",
 * "Successfully loaded shootercalib slot " + selectedSlot);
 * }
 * else
 * {
 * Logging.stickyWarning("Shooter Calibration Warning",
 * "Shooter calibration not found. Shooter won't be able to shoot.");
 * }
 * }
 * 
 * // Motors
 * upperMotor = new SparkMax(Constants.ShooterConstants.UpperShooterMotorCANID,
 * MotorType.kBrushless); // MAXMotion
 * // Velocity
 * SparkMaxConfig upperMotorConfig = new SparkMaxConfig();
 * upperMotorConfig.closedLoop.feedForward.kV(Constants.ShooterConstants.
 * UpperShooterPF_F);
 * upperMotorConfig.closedLoop.p(Constants.ShooterConstants.UpperShooterPF_P);
 * upperMotor.configure(upperMotorConfig, ResetMode.kNoResetSafeParameters,
 * PersistMode.kPersistParameters);
 * 
 * lowerMotor = new SparkMax(Constants.ShooterConstants.LowerShooterMotorCANID,
 * MotorType.kBrushless); // MAXMotion
 * // Velocity
 * SparkMaxConfig lowerMotorConfig = new SparkMaxConfig();
 * lowerMotorConfig.closedLoop.feedForward.kV(Constants.ShooterConstants.
 * LowerShooterPF_F);
 * lowerMotorConfig.closedLoop.p(Constants.ShooterConstants.LowerShooterPF_P);
 * lowerMotor.configure(lowerMotorConfig, ResetMode.kNoResetSafeParameters,
 * PersistMode.kPersistParameters);
 * 
 * feederMotor = new SparkMax(Constants.ShooterConstants.FeederMotorCANID,
 * MotorType.kBrushless); // Velocity
 * SparkMaxConfig feederMotorConfig = new SparkMaxConfig();
 * feederMotorConfig.idleMode(IdleMode.kBrake);
 * feederMotorConfig.closedLoop.feedForward.kV(Constants.ShooterConstants.
 * FeederPF_F);
 * feederMotorConfig.closedLoop.p(Constants.ShooterConstants.FeederPF_P);
 * feederMotor.configure(feederMotorConfig, ResetMode.kNoResetSafeParameters,
 * PersistMode.kPersistParameters);
 * 
 * 
 * refreshShooterTargetRPMs();
 * 
 * distCalcModeChooser.setDefaultOption("Vision",
 * ShooterDistCalcMode.VISION_BASED);
 ...
 * if (selectedSlot == -1) {
 * Logging.stickyError("Shooter Calibration Error",
 * "An error occured while reading the selected shootercalib slot during reload"
 * );
 * return;
 * }
 * 
 ...
 * calibration = new ShooterCalibration();
 * Logging.stickyWarning("Shooter Calibration Warning",
 * "Failed to reload shooter calibration slot " + selectedSlot +
 * ". Using empty calibration.");
 * } else {
 * calibration = loadedCalibration;
 * Logging.infoMsg("Shooter Calibration Reloaded", "Reloaded shootercalib slot "
 * + selectedSlot);
 * }
 * 
 * refreshShooterTargetRPMs();
 ...
 * // BOOL CHECKS
 * private boolean isFeederAtTargetRPM() {
 * return Math.abs(getFeederMotorRPM() -
 * Constants.ShooterConstants.Feeder_TargetRPM) <
 * Constants.ShooterConstants.RPM_Tolerance;
 * }
 * 
 * private boolean isShooterAtTargetRPM() {
 * return Math.abs(getUpperMotorRPM() - upperMotorTargetRPM) <
 * Constants.ShooterConstants.RPM_Tolerance && Math.abs(getLowerMotorRPM() -
 * lowerMotorTargetRPM) < Constants.ShooterConstants.RPM_Tolerance;
 * }
 * 
 * // GET RPMS
 * public double getUpperMotorRPM() {
 * return Robot.isSimulation() ? upperMotorSimRPM :
 * upperMotor.getEncoder().getVelocity();
 * }
 * 
 * public double getLowerMotorRPM() {
 * return Robot.isSimulation() ? lowerMotorSimRPM :
 * lowerMotor.getEncoder().getVelocity();
 * }
 * 
 * private double getFeederMotorRPM() {
 * return Robot.isSimulation() ? feederMotorSimRPM :
 ...
 * 
 * if(feederState && feederUnlockAfter < now)
 * feederMotor.getClosedLoopController().setSetpoint(Constants.ShooterConstants.
 * Feeder_TargetRPM, ControlType.kVelocity);
 * else
 * feederMotor.getClosedLoopController().setSetpoint(0, ControlType.kVelocity);
 * 
 * 
 ...
 * SmartDashboard.putNumber("Shooter/RPMs/FeederRPM", getFeederMotorRPM());
 * 
 * SmartDashboard.putNumber("Shooter/TargetRPMs/UpperShooterTargetRPM",
 * upperMotorTargetRPM);
 * SmartDashboard.putNumber("Shooter/TargetRPMs/LowerShooterTargetRPM",
 * lowerMotorTargetRPM);
 * 
 * SmartDashboard.putNumber("Shooter/CalibrationSlot", calibration == null ? -1
 * : calibration.getSlot());
 * 
 * // NOTE: Only shooter distance to hub is influenced by the manual distance.
 * Robot distance to hub is always calculated with the drivetrain pose.
 * SmartDashboard.putNumber("Shooter/ShooterDistanceToHub",
 * shooterDistanceToHub);
 * SmartDashboard.putNumber("Shooter/RobotDistanceToHub", robotDistanceToHub);
 * }
 * 
 * @Override
 * public void simulationPeriodic() {
 * if(Math.abs(getUpperMotorRPM() - upperMotorTargetRPM) >
 * Constants.ShooterConstants.RPM_Tolerance)
 * upperMotorSimRPM +=
 * Math.signum(upperMotor.getClosedLoopController().getSetpoint()-
 * upperMotorSimRPM) * 0.02 * 4000;
 * 
 * if(Math.abs(getLowerMotorRPM() - lowerMotorTargetRPM) >
 * Constants.ShooterConstants.RPM_Tolerance)
 * lowerMotorSimRPM +=
 * Math.signum(lowerMotor.getClosedLoopController().getSetpoint()-
 * lowerMotorSimRPM) * 0.02 * 4000;
 * 
 * feederMotorSimRPM = feederMotor.getClosedLoopController().getSetpoint();
 * }
 * 
